  ...
  <component :is="field.fullSize ? 'full-width-field' : 'default-field'" :field="field" full-width-content>
    <template #field>
      <div :class="{'px-8 pt-6': field.fullSize}">
        <gallery #value ref="gallery" v-if="hasSetInitialValue"
                 v-model:value="value" :editable="!field.readonly" :removable="field.removable" custom-properties
                 :field="field" :multiple="field.multiple" :uploads-to-vapor="field.uploadsToVapor"
                 :has-error="hasError" :first-error="firstError"/>

        <div v-if="field.existingMedia">
          <button type="button" class="form-file-btn btn btn-default btn-primary mt-2"
...
export default {
  ...
  props: ['resourceName', 'resourceId', 'field', 'index', 'related-resource-name', 'related-resource-id', 'showErrors'],
  emits: ['fieldChanged', 'fileDeleted', 'fileUploadStarted', 'fileUploadFinished'],
  data() {
    return {
