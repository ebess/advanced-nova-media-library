<template>
  <gallery-item :class="{ 'show-statistics': field.showStatistics, 'show-name':field.showName }"
                class="gallery-item-image">
    <div class="gallery-item-info p-3">
      <a v-if="downloadUrl" :href="downloadUrl" class="icon download" title="Download">
        <Icon height="16" type="download" view-box="0 0 20 22" width="16"/>
      </a>
      <a v-if="removable" class="icon delete" href="#" title="Remove" @click.prevent="$emit('remove')">
        <Icon height="16" type="trash" view-box="0 0 20 20" width="16"/>
      </a>
      <a v-if="isCustomPropertiesEditable" class="icon edit" href="#" title="Edit custom properties"
         @click.prevent="$emit('edit-custom-properties')">
        <Icon height="16" type="pencil" view-box="0 0 20 20" width="16"/>
      </a>
      <a class="preview" href="#" @click.prevent="showPreview">
        <Icon height="30" type="search" view-box="0 0 20 20" width="30"/>
      </a>
      <a v-if="croppable" class="icon crop" href="#" @click.prevent="$emit('crop-start', image)">
        <scissors-icon brand="var(--colors-black)" height="16" view-box="0 0 20 20" width="16"/>
      </a>
    </div>
    <img ref="image" :alt="image.name" :src="src" class="gallery-image">
    ...
    <span v-if="field.showName" class="name">
      {{ image.file_name }}
    </span>
